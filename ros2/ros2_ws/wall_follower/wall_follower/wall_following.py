...
class WallFollower(Node):
    ...
    def get_result_callback(self, future):
        ...
        self.get_logger().info(f'Odometry recording completed: {result.list_of_odoms}')
...
def send_stop_command(node):
    """Sends a zero velocity command to stop the robot."""
    twist = Twist()
    twist.linear.x = 0.0
    twist.angular.z = 0.0
    node.publisher.publish(twist)
    node.get_logger().info('Sent stop command to the robot.')
...
def main(args=None):
    ...
    def signal_handler(sig, frame):
        ...
        wall_follower.get_logger().info('SIGINT caught, stopping the robot...')
        send_stop_command(wall_follower)
    ...
    try:
        ...
        wall_follower.get_logger().error('An error occurred: {}'.format(e))
        send_stop_command(wall_follower)
    finally:
        wall_follower.destroy_node()
        rclpy.shutdown()
